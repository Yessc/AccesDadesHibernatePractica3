...
public class Manager {
    ...
    public static void updateAutor(Long autorId, String nom, Set<Llibre> llibres) {
        ...
        try {
            ...
            // Actualizamos datos simples
            autor.setNom(nom);

            // 1Limpiar relaciones antiguas (lado propietario Llibre)
            for (Llibre oldLlibre : autor.getLlibres()) {
                Llibre oldLlibreDB = session.find(Llibre.class, oldLlibre.getLlibreId());
                if (oldLlibreDB != null) oldLlibreDB.getAutors().remove(autor);
            }
    ...
    public static Prestec addPrestec(Exemplar exemplar, Persona persona, LocalDate dataPrestec, LocalDate dataRetornPrevista) {
        ...
        try {
            ...
            if (exemplarDB != null && exemplarDB.isDisponible()){
                ...
                System.out.println("L'exemplar "+ exemplar.getCodiBarres()+ "no dispo");
            }
    ...
    public static void registrarRetornPrestec(Long prestecId, LocalDate dataReal) {
        ...
        try {
            ...
            //  Recuperar el préstec
            

            // Comprovar que existeix i està actiu
            ...
            if (prestec != null && prestec.isActiu()) {
                ...
                System.out.println("Préstec no existeix o ja ha estat retornat.");
            }
    ...
    public static List<Object[]> findLlibresEnPrestec() {
        try (Session session = factory.openSession()) {
            ...
            String hql = "SELECT e.llibre.titol, p.persona.nom " +
                    "FROM Prestec p " +
                    "JOIN p.exemplar e " +
                    "WHERE p.actiu = true";
    ...
    public static List<Object[]> findLlibresAmbBiblioteques() {
        try (Session session = factory.openSession()) {
            ...
            String hql = "SELECT e.llibre.titol, e.biblioteca.nom " +
                     "FROM Exemplar e";
            return session.createQuery(hql, Object[].class).list();
        } catch (Exception e) {
            e.printStackTrace();
